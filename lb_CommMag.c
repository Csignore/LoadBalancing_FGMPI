...
int FG_Process(int argc, char **argv){
    ...
    if (mag_color) {
    	...
    	MPI_Request request,helprequest;
        int prevmag, nextmag;
    	...
    	while (notdoneSign){
            ...
            // TROUBLE!!
            int helpResult = FALSE;
            MPI_Test(&helprequest,&result,&status);
            if ( helpResult == TRUE) {
                replyreqbuf = (MAXINT - replyreqbuf)/2 + replyreqbuf;
            ...
            if (reqbuf == -1) {
                ...
                int help = TRUE;
                MPI_Send(&help, 1, MPI_INT, nextmag, HELP_TAG, mag_comm);
                MPI_Recv(&replyreqbuf, 1, MPI_INT, nextmag, HELP_TAG, mag_comm);
                if (replyreqbuf == -1){notdoneSign = FALSE;
...
int who_are_my_workers(int rank, int startRank, int osProcSize, int *prevworker_ptr, int *nextworker_ptr)
{
    ...
    nextworker = (startRank + osProcSize-1==rank) ? startRank : (rank-startRank+1);
...
int who_are_my_magNeighb(int rank, int osProcNum, int osProcSize, int *prevmag_ptr, int *nextmag_ptr)
{
    ...
    prevmag = (0 ==rank) ? (osProcNum-1) : (rank- osProcSize);
    nextmag = ((osProcNum-1)*osProcSize ==rank) ? 0 : (rank+osProcSize);
